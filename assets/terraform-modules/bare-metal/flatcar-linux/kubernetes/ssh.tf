...
resource "null_resource" "copy-controller-secrets" {
  ...
  provisioner "remote-exec" {
    inline = [
      "set -e",
      "sudo cp $HOME/kubelet.config /etc/kubernetes/kubelet.config",
      "sudo chown root:root /etc/kubernetes/kubelet.config",
      "sudo chmod 600 /etc/kubernetes/kubelet.config",
